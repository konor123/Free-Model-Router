...
OpenCode Zen의 `/models`는 무료 모델만 나열하는 목록이 아닙니다. FMR의 네이티브
OpenCode 카탈로그에는 공식 검증된 무료 모델 ID 6개만 허용하며, Zen 키가 설정된 경우에도
이 정책은 동일하게 적용됩니다. Zen 키 사용 시 공개 경로와 Zen 경로 모두 같은 무료 모델에만
제공되며, 확인되지 않은 신규·알 수 없는 ID는 안전하게 거부됩니다.
