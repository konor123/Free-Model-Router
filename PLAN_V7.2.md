...
## Latency Measurement Granularity

Latency는 **provider 평균이 아니라 `ProviderModelID × RouteID` 단위**로 측정한다.

```text
Latency key = ProviderModelID × RouteID
```

예:

```text
opencode/mimo-v2.5 × public
opencode/glm-5.3-flash × public
opencode/mimo-v2.5 × zen-auth
openrouter/mimo-v2.5 × default
```

같은 provider 안에서도 모델별 latency가 크게 다르므로 provider-level average latency를 model routing에 사용하지 않는다. Provider-level latency는 장애 감지나 UI 요약 통계에만 사용할 수 있다.

Routing score의 latency component는 항상 해당 ProviderModel + Route의 TTFT를 사용한다.

\[
R_{m,r}=0.70P_m+0.30L_{m,r}
\]
...
```text
same provider / different models → independent TTFT
same model / different routes → independent TTFT
provider average is never used for model ranking
...
Core router가 먼저 완성된 뒤 **API key 없이 조회 가능한 공개 benchmark sources**를 조합해 performance score를 추가한다.

## 기본 원칙

```text
No mandatory API key
Public benchmark sources first
Source adapters independent
Missing source tolerated
Last-known-good local cache
```

Artificial Analysis는 기본 의존성에서 제외한다.

```text
Artificial Analysis
→ optional secondary source only
```
...
SWE-bench
→ Agentic SWE evidence

Hugging Face OpenEvals / leaderboard-data
→ 공개 benchmark aggregation / general evidence
```

OpenEvals는 독립 benchmark가 아니라 여러 공개 benchmark 결과를 통합하는 **보조 aggregation source**로 취급한다.
...
type BenchmarkSource interface {
    ID() string
    Fetch(ctx context.Context) ([]BenchmarkResult, error)
}
```
...
특정 source 장애가 FMR router를 중단시키면 안 된다.
...
각 source에 stable model ID가 있으면 `SourceModelID`에 저장한다.
...
Agentic A
└─ SWE-bench
   (agent harness / scaffold 영향 때문에 confidence 조정)

Intelligence / Reasoning I
├─ LiveBench Reasoning
└─ OpenEvals general benchmark evidence
```

각 source raw score는 가능한 한 동일 leaderboard snapshot 내부 percentile 0–100으로 변환한다.

\\[
C = 0.7C_{Aider} + 0.3C_{LiveBench}
\\]
...
Missing source/component는 존재하는 weight만 재정규화한다.
...
```bash
go test ./internal/benchmark/... ./internal/matcher/... ./internal/scoring/...
```

## 완료 조건

- API key 없이 기본 performance scoring이 동작한다.
- 특정 benchmark source 장애가 router를 중단시키지 않는다.
- benchmark 데이터가 전혀 없어도 neutral prior로 routing이 계속된다.
- SWE-bench처럼 harness 영향을 받는 score를 순수 model benchmark와 동일 confidence로 취급하지 않는다.
...
Model-route-specific latency
Never rank models using provider-average latency
