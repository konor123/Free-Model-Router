...
// ChatCompletion posts the request to the AI Studio route and returns a ChatStream.
// All routes share one credential: the GEMINI_API_KEY bearer token.
func (p *Provider) ChatCompletion(ctx context.Context, route model.ProviderRoute, req provider.NormalizedRequest) (provider.ChatStream, error) {
	...
	// guarantees drain/close on every error path below.
...
func buildPayload(mdl string, req provider.NormalizedRequest) ([]byte, error) {
	...
	for _, t := range req.Tools {
		if t.Type != "" && t.Type != "function" {
			return nil, &provider.UnsupportedError{Parameter: "tools[].type", Reason: "only function tools are supported by the gemini ai route"}
		}
	...
	for _, ext := range req.Extensions {
		// Gemini rejects unknown params explicitly (never silently drop).
		return nil, &provider.UnsupportedError{Parameter: ext.Name, Reason: "not supported by the gemini ai route"}
	}
...
func (s *sseStream) Next(ctx context.Context) (provider.StreamEvent, error) {
	...
	for {
		...
		if err != nil {
			s.done = true
			// ReadString can return a final unterminated line together with
			// io.EOF; process it before ending the stream.
			...
			// Fall through: parse the trailing line, then report EOF next call.
		}
		line = strings.TrimRight(line, "\r\n")
		if line == "" || strings.HasPrefix(line, ":") {
			if err != nil { // trailing blank line at EOF
				s.done = true
				return provider.StreamEvent{}, io.EOF
			}
