...
// newMockServer builds a test xAI-compatible endpoint.
...
func TestDiscoverModelsCatalog(t *testing.T) {
	...
	_, p := newMockServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(catalogResponse{Data: []catalogEntry{
			{ID: "grok-4.6", ContextLength: 256000, Streaming: &streaming, Tools: &tools, Vision: &vision},
			{ID: "grok-3-mini"},
		}})
	})
	...
	if pm.Base.ContextLength != 256000 {
...
func TestChatCompletionNonStreaming(t *testing.T) {
	...
	route := APIRoute(mustID(t, "xai", "grok-4.6"), "grok-4.6", model.Capabilities{})
...
func TestChatCompletionStreamingSSE(t *testing.T) {
	t.Setenv(EnvKey, "test-key")
	// Last data line has no trailing newline: the trailing-line EOF path.
	...
	if count != 2 {
		t.Fatalf("expected 2 events (2 text deltas incl. unterminated last line), got %d", count)
	}
}
...
func TestSSEPreservesToolCallsAndReasoning(t *testing.T) {
	...
	stream, err := p.ChatCompletion(context.Background(), APIRoute(mustID(t, "xai", "m"), "m", model.Capabilities{}), provider.NormalizedRequest{Stream: true, Messages: []provider.Message{{Role: "user", Content: "hi"}}})
	if err != nil {
		t.Fatal(err)
	}
	defer stream.Close()
