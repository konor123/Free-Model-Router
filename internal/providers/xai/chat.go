...
// ChatCompletion posts the request to the xAI route and returns a ChatStream.
// All routes share one credential: the XAI_API_KEY bearer token.
...
func buildPayload(mdl string, req provider.NormalizedRequest) ([]byte, error) {
	...
	for _, t := range req.Tools {
		if t.Type != "" && t.Type != "function" {
			return nil, &provider.UnsupportedError{Parameter: "tools[].type", Reason: "only function tools are supported by the xai api route"}
		}
	...
	for _, ext := range req.Extensions {
		// xAI rejects unknown params explicitly (never silently drop).
		return nil, &provider.UnsupportedError{Parameter: ext.Name, Reason: "not supported by the xai api route"}
	}
