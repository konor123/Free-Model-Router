...
func (p *Provider) ChatCompletion(ctx context.Context, route model.ProviderRoute, req provider.NormalizedRequest) (provider.ChatStream, error) {
	...
	// Non-streaming: parse whole body into one synthetic stream.
...
func buildPayload(mdl string, req provider.NormalizedRequest) ([]byte, error) {
	...
	for _, ext := range req.Extensions {
		// OpenCode Public rejects unknown params explicitly (PLAN_V7: never silently drop).
		return nil, &provider.UnsupportedError{Parameter: ext.Name, Reason: "not supported by opencode public route"}
	}
