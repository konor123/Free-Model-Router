...
// newMockServer builds a test NVIDIA-compatible endpoint.
...
func TestDiscoverModelsCatalog(t *testing.T) {
	...
	_, p := newMockServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(catalogResponse{Data: []catalogEntry{
			{ID: "meta/llama-3.1-8b-instruct", ContextLength: 131072, Streaming: &streaming, Tools: &tools},
	...
	// Slash-bearing upstream IDs use the b64 segment convention.
	id, err := model.NewProviderModelID("nvidia", "b64-bWV0YS9sbGFtY")
	if err != nil {
		t.Fatal(err)
	}
	_ = id
	var found *model.ProviderModel
	for _, pm := range snap.Models {
		if pm.UpstreamID == "meta/llama-3.1-8b-instruct" {
			found = &pm
			break
		}
	}
	if found == nil {
		t.Fatal("missing meta/llama-3.1-8b-instruct in catalog")
	}
	if found.Base.Tools != true || found.Base.Streaming != true {
		t.Fatalf("capabilities not normalized: %+v", found.Base)
	}
	if found.Base.ContextLength != 131072 {
		t.Fatalf("context length not normalized: %d", found.Base.ContextLength)
	}
	if !found.Base.IsUnknown(model.CapVision) || !found.Base.IsUnknown(model.CapStructuredOutput) || !found.Base.IsUnknown(model.CapReasoning) {
		t.Fatalf("omitted feature metadata must remain unknown: %+v", found.Base)
	}
}
...
func TestChatCompletionNonStreaming(t *testing.T) {
	...
	route := HostedRoute(mustID(t, "nvidia", "b64-bWV0YS9sbGFtY"), "meta/llama", model.Capabilities{})
...
func TestSSEPreservesToolCallsAndReasoning(t *testing.T) {
	...
	stream, err := p.ChatCompletion(context.Background(), HostedRoute(mustID(t, "nvidia", "m"), "m", model.Capabilities{}), provider.NormalizedRequest{Stream: true, Messages: []provider.Message{{Role: "user", Content: "hi"}}})
	if err != nil {
		t.Fatal(err)
	}
	defer stream.Close()
